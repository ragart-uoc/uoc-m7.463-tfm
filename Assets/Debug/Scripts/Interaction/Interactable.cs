...
namespace TFM.Debug.Scripts.Interaction
{
    ...
    public class Interactable : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        /// <value>Property <c>itemInteractions</c> represents the item interactions.</value>
        public ItemInteraction[] itemInteractions;
        ...
        private void Update()
        {
            _isInteractionPossible = !UIManager.Instance.radialMenu.gameObject.activeSelf
                                     || UIManager.Instance.radialMenu.target == transform;
            Highlight();
        }
        ...
        {
            foreach (var itemInteraction in itemInteractions)
            {
                if (itemInteraction.item != item)
                    continue;
                itemInteraction.actionSequence.ExecuteSequence();
                ItemManager.Instance.DiscardItem(item);
                return;
            }
            StartCoroutine(UIManager.Instance.ShowMessage("That doesn't work.", 3f));
        }
    }
}
